...
const ElevesAttendus1 = () => {
  ...
  useEffect(() => {
    fetchElevesInscrits();
  }, [searchText, anneescolairesID, drenasID]);
  ...
  const fetchElevesInscrits = () => {
    db.transaction(tx => {
      let query = `
        ...
        WHERE ei.anneescolaires_id = ? AND et.drenas_id = ?
      `;
    ...
    OR UPPER(COALESCE(e.nomeleve,'') || ' ' || COALESCE(e.prenomseleve,'')) LIKE ?
      ...
      tx.executeSql(
        ...
        (_, result) => {
          ...
          setEleves(rows);
          console.log('Élèves récupérés:', rows);
        },
        (_, error) => {
          console.error(
          ...
          if (error) {
            ...
            console.error('Requête SQL échouée:', error.sql);
          }
        },
      );
    });
  ...
  return (
    <View style={styles.container}>
      {/* 🟢 Bande de notification du total */}
...
const styles = StyleSheet.create({
  ...
  notification: {
    backgroundColor: '#2196f3', // bleu pour "attendus"
    padding: 10,
    borderRadius: 8,
    marginBottom: 10,
    alignItems: 'center',
  ...
  count: {
    color: '#ffd700', // jaune pour bien mettre en valeur le total
  },
  searchInput: {
    height: 40,
    borderColor: 'gray',
...
export default ElevesAttendus1;
