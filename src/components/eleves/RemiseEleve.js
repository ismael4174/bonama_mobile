...
const ElevesInscrits = ({navigation}) => {
  ...
  const [remiseEleveDetailsVisible, setRemiseEleveDetailsVisible] =
    useState(false);
  const [selectedEleve, setSelectedEleve] = useState(null);
  const [remiseEleveDetailsEditable, setRemiseEleveDetailsEditable] =
    useState(true);
  ...
  const handleFinaliserRemise = useCallback(item => {
    ...
    setRemiseEleveDetailsVisible(true);
  }, []);

  const handleEditerRemise = useCallback(item => {
    setSelectedEleve(item);
    setRemiseEleveDetailsEditable(item.remisefinalise === 0);
    setRemiseEleveDetailsVisible(true);
  }, []);
  ...
  const closeRemiseEleveDetails = useCallback(() => {
    setRemiseEleveDetailsVisible(false);
  }, []);
  return (
    <View style={styles.container}>
      ...
      />
      <Modal
        visible={remiseEleveDetailsVisible}
        onRequestClose={closeRemiseEleveDetails}
        transparent={true}
        animationType="slide">
        <View style={styles.modalContainer}>
          <View style={styles.modal}>
            <RemiseEleveDetails
              eleve={selectedEleve}
              editable={remiseEleveDetailsEditable}
              onClose={closeRemiseEleveDetails}
